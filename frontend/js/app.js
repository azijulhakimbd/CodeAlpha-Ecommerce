...
function addToCart(p) {
  ...
  if (existing) {
    if (existing.quantity >= p.stock)
      return toast("Maximum available stock reached");
    existing.quantity++;
  } else
    state.cart.push({
      productId: p._id,
...
async function initCheckout() {
  ...
  $("#checkoutSummary").innerHTML =
    `<h3>Order summary</h3>${state.cart.map((x) => `<div class="summary-line"><span>${escapeHtml(x.name)} × ${x.quantity}</span><b>${money(x.price * x.quantity)}</b></div>`).join("")}<div class="summary-line total"><span>Total</span><b>${money(total)}</b></div>`;
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      state.cart = [];
      saveCart();
      location.href = `/orders.html?success=${order._id}`;
    } catch (err) {
...
async function initOrders() {
  const root = $("#ordersView");
  if (!root) return;
  if (!state.user) {
    root.innerHTML = `<div class="empty">Please <a href="/login.html">sign in</a> to view your orders.</div>`;
    return;
  }
  try {
    ...
    root.innerHTML = orders.length
      ? orders
          .map(
            (o) =>
              `<article class="order"><div class="order-head"><div><b>Order #${String(o._id).slice(-8).toUpperCase()}</b><div class="order-items">${new Date(o.createdAt).toLocaleString()}</div></div><span class="status">${o.status}</span></div><div class="order-items">${o.items.map((i) => `${escapeHtml(i.name)} × ${i.quantity}`).join(" · ")}</div><strong>Total: ${money(o.totalAmount)}</strong></article>`,
          )
          .join("")
      : `<div class="empty">No orders yet. <a href="/">Shop now →</a></div>`;
  } catch (e) {
    root.innerHTML = `<div class="empty">${e.message}</div>`;
  }
}
